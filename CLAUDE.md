...
- **`HandVisibilityController`** — hotseat hand privacy. Created at runtime by `CardActionExecutor.Awake` (`AddComponent`, like `NeighManager`/`BoardChrome`), no scene wiring. Every `LateUpdate` it computes the set of players whose hand should be face-up and toggles each hand card's `cardFront`/`cardBack` (only when the state differs). Face-up: the current `turnManager.activePlayer` (always); the current Neigh `Responder` while `NeighManager.AwaitingResponse`; the `pendingTakeFromHandTargetPlayer` while a `TakeFromHand` action is pending; a Peeping Narwhal target on the caster's next turn (a 3-state `peepPhase` machine — cast turn → intervening turn → reveal turn → cleared). A prompt that reassigns `activePlayer` to a non-active player (`DiscardCardAction`/`GiveCardAction` targeting the opponent) reveals that hand for free via the active-player rule, which is correct. No gameplay logic reads a card's face — this is display only. `CardHoverZoom` also gates its zoom on `cardFront.activeSelf` so face-down cards don't lift.
...
| `DestroyCardAction` | `destroyer` (ActivePlayer/Opponent), `targetStable` (Any/Unicorn/Upgrade), `numberOfCards`, `destroyAll` (bool) | Destroyer picks N cards from the opposing player's stables and sends them to the discard pile. `targetStable=Any` allows unicorn/upgrade/downgrade; `targetStable=Unicorn` restricts to unicorn cards only (uses `PendingActionType.DestroyUnicornCard`); `targetStable=Upgrade` restricts to upgrade cards only (uses `PendingActionType.DestroyUpgradeCard`, mirroring the Unicorn path — `Stable.HandleCardClick` requires `this is UpgradeStable`). `destroyAll=true` mirrors `SacrificeCardAction.sacrificeAll`: every card in scope is moved to discard immediately, no prompt, `numberOfCards` ignored — and sacrifice shields (`ISacrificeShield`) are **not** consulted (a mass destroy has no single target to intercept). Used by Buck Naked (`targetStable=Upgrade, destroyAll=true`) and Kink Shame (`targetStable=Upgrade`, interactive). |
| `PullCardAction` | `numberOfCards`, `skipDrawPhaseOnSuccess` (bool) | Randomly moves N cards from the **opponent's** hand to the **active player's** hand. Capped at opponent's hand size. If `skipDrawPhaseOnSuccess=true` and ≥1 card was pulled, calls `TurnManager.RequestSkip(activePlayer, TurnSkip.Draw)` to skip their Draw phase. |
| `SacrificeCardAction` | `sacrificer` (ActivePlayer/Opponent), `targetStable` (Unicorn/Upgrade/Downgrade/Any), `sacrificeAll` (bool), `numberOfCards`, `targetSubtype` (`UnicornType?`) | Moves cards from the **sacrificer's own** stables to the discard pile. `sacrificer` defaults to `ActivePlayer` (every caller before Sex, Drugs, and Unicorns, which uses `Opponent`). `sacrificeAll=true` auto-moves all matching cards immediately, no prompt. `sacrificeAll=false` prompts the sacrificer to click a card in one of their own stables matching `targetStable`/`targetSubtype` (mirrors `DiscardCardAction`'s `PlayerChooses` mode); skips silently (no prompt) if no eligible card exists anywhere in scope. `targetSubtype` works like `StealUnicornAction.targetSubtype` and is only meaningful when `targetStable` includes the Unicorn stable. |
...
| `LookAndTakeFromHandAction` | `targetPlayer` (Opponent/ActivePlayer), `destination` (`ActivePlayerHand`/`DiscardPile`), `cardTypeFilter` (`CardType?`) | "Look at another player's hand, choose 1 card" — the chooser (active player) clicks a card in the target player's `HandStable`, which moves to their own hand (or straight to the discard pile). Every hand already renders face-up for the player looking (see `HandVisibilityController`), so there's no reveal step. Modelled on `DestroyCardAction`: records `pendingTakeFromHandTargetPlayer`/`pendingTakeFromHandTypeFilter` on the executor and prompts with `PendingActionType.TakeFromHand`; `HandStable.HandleCardClick` has the matching branch, checked **before** its active-player guard (the clicked hand belongs to the target, not the chooser). `cardTypeFilter` set + nothing matches ⇒ skips silently ("you may"). Used by Hoof Job (any card → hand), Entitled Unicorn (`UNICORN` → hand), Officer Hornie (`UNICORN` → discard). |
| `RevealHandsAction` | none | Peeping Narwhal: "each other player must show you their hand." No cards move. Calls `HandVisibilityController.RevealOnCasterNextTurn(caster, target)` for each other player — casting a Unicorn ends the caster's turn immediately, so the reveal is deferred to the caster's next turn, then auto-cleared. Also logs each hand. |
| `ChooseEffectAction` | `chooser` (ActivePlayer/Opponent), `optionALabel`/`optionA` (`List<CardAction>`)/`optionAViable` (`Predicate<CardActionContext>`), same for B | "Choose between two effects." Evaluates the two viability predicates against the live context: **0 viable** ⇒ silent no-op; **1 viable** ⇒ that option's actions are run with no prompt (`CardActionExecutor.PrependActions` splices them onto the front of the queue); **2 viable** ⇒ `PromptEffectChoice` sets `PendingActionType.ChooseEffect` and reassigns `activePlayer` to the chooser; `EffectChoicePanel` renders the two buttons and calls `CardActionExecutor.ResolveEffectChoice(0|1)`, which prepends the picked list and resumes. Viability is what keeps a `numberOfCards>1` sub-action (Sex, Drugs, and Unicorns' "discard 3") from being offered into a soft-lock. Predicates + nested action lists are runtime-only (rebuilt in `OnEnable`, not serialized). Used by Kink Shame (caster chooses) and Sex, Drugs, and Unicorns (opponent chooses). |
| `ForEachPlayerAction` | `actions` (`List<CardAction>`) | "Each player (including you) must …" Runs the template once per player, in seat order starting with the current "you". Each pass is bound to a context from `CardActionContext.ForPlayer(player, nextInSeat)` — `activePlayer` = that player — so any existing action targeting `ActivePlayer` means "this player". All passes are spliced onto the queue front at once (`PrependActions` per-action-context overload); prompts pause the loop like any top-level prompt. `context.caster` is never rebound. Template action instances are shared between passes — no per-run state in fields. Used by Cult Leader Unicorn, Safe Sex, Unicorn Acid Trip, Rainbow Shitstorm. |
| `ReturnBabyToNurseryAction` | none | Moves one Baby Unicorn from the active player's Unicorn stable to the Nursery; no prompt (all Babies share one `CardData`). Skips silently if none. Used by Safe Sex. |
| `DiscardHandAction` | `redrawSameCount` (bool) | Discards the active player's whole hand, no prompt, **skipping `context.sourceCard`** (a Magic card is still in hand while its effect runs). `redrawSameCount` then draws that many back. Used by Unicorn Acid Trip (`true`) and Rainbow Shitstorm. |
| `ShuffleDiscardIntoDeckAction` | none | Moves the whole discard pile into the play deck (any Baby Unicorn → Nursery) and shuffles. Used by Rainbow Shitstorm. |
| `ReturnToHandAction` | `whose` (Self/OtherPlayers), `targetStable` (Any/Unicorn) | Active player clicks a card in an eligible player's stable; it goes to **that stable owner's** hand (resolved per click in `ExecutePendingAction`, `PendingActionType.ReturnToHand`). `pendingReturnEligibleOwners`/`pendingReturnUnicornOnly` gate the click in `Stable.HandleCardClick`. Skips silently if nothing is eligible. A Baby Unicorn goes to the Nursery instead (`MoveCard`). Used by Sextra-Terrestrial Unicorn (OtherPlayers/Any), Unicorn Flatulence (Self/Unicorn). |
| `MoveUnicornAction` | `source` (Self/AnyPlayer), `returnAtEndOfTurn` (bool) | Two steps: click a Unicorn in an eligible stable (`PendingActionType.MoveUnicorn`), then — only if >1 legal destination — pick a player (`ChoosePlayer`). Destinations = everyone except the card's owner and the mover (`CardActionExecutor.GetMoveDestinations`); with 2 players there is always exactly one, so step 2 never shows. `returnAtEndOfTurn` schedules the card's return via `TurnManager.ScheduleAtEndOfTurn(mover, …)`, only if it's still in the stable it was loaned to. `EligibleSources` doubles as the card's `CanActivateEveryTurn` gate. Used by Dominatrix Whip (AnyPlayer), Unicorn Cuckold (Self, loan). |
| `SkipPhaseAction` | `target` (ActivePlayer/Opponent), `kind` (`TurnSkip`: Draw/Action/Turn) | Calls `TurnManager.RequestSkip(player, kind)` — no prompt. Used by Sticky Situation (via `ChooseEffectAction`), Unicorn Hangover (inside `ChoosePlayerAction`). |
| `ChoosePlayerAction` | `title`, `then` (`List<CardAction>`) | "Choose a player. That player …" The active player picks another player — auto-picked with one candidate (always, with 2 players), otherwise the `ChoosePlayer` prompt — and `then` runs with `context.opponentPlayer` rebound to the chosen one (`ForPlayer(active, chosen)` + `PrependActions`), so existing Opponent-targeting actions act on them unchanged. Used by Unicorn Hangover; the older "choose a player" cards (e.g. Sex, Drugs, and Unicorns) still hardcode the opponent and could migrate to it. |
...
Each queued action carries its own `CardActionContext` (`Queue<(CardAction, CardActionContext)>`); `ExecuteNextAction` makes the dequeued action's context current. A plain `ExecuteActions` call tags every action with one context built from the caster; `ForEachPlayerAction` binds per-player copies. `CardActionContext.caster` is the player who played the source card and is never rebound.
...
`pendingDestroyTargetPlayer` is set by `DestroyCardAction` before prompting and records exactly which player's stable cards may be selected from. `Stable.HandleCardClick` checks `player == pendingDestroyTargetPlayer` to accept or reject a click — this is more reliable than comparing against `turnManager.activePlayer`, which can be temporarily reassigned during multi-step action sequences. `pendingSacrificeTargetPlayer` is the same pattern for `SacrificeCardAction`'s `PlayerChooses` mode (`PendingActionType.SacrificeCard`); `pendingSacrificeTargetStable`/`pendingSacrificeSubtypeFilter` additionally restrict which stable type and unicorn subtype are valid to click, mirroring `pendingStealSubtypeFilter`. `pendingTakeFromHandTargetPlayer`/`pendingTakeFromHandTypeFilter` do the same for `LookAndTakeFromHandAction` (`PendingActionType.TakeFromHand`) — which player's `HandStable` may be clicked, and an optional `CardType` filter on the pickable card. `pendingChoice{Title,LabelA,LabelB,OptionA,OptionB}` back `ChooseEffectAction` (`PendingActionType.ChooseEffect`); unlike the others these aren't click-routing state — `EffectChoicePanel` reads them to render its two buttons, and `ResolveEffectChoice` consumes them. `PromptPlayerChoice(chooser, title, players, onChosen)` / `ResolvePlayerChoice(i)` are the generic "choose a player" prompt (`PendingActionType.ChoosePlayer`, `pendingPlayerChoices`) — `EffectChoicePanel` renders one button per player; the callback runs, then the queue resumes. `ClearPendingAction` now delegates the field reset to `ResetPendingState()` so `ResolveEffectChoice` can reuse it without the queue-resume that `ClearPendingAction` does.
...
| `ISelfSacrificeCondition` | `bool ShouldSacrifice(Player owner)` | `CardManager.MoveCard` (after every move) | "If at any time …, SACRIFICE this card." After each move, every card in every stable implementing this is checked with its stable's owner; true ⇒ moved to the discard pile. Re-entry guarded; re-scans until stable. Used by Unicorn Flatulence (owner has no Unicorns). |
...
- **`CardHoverZoom`** — on the `Card` prefab. Lifts + scales a card ~1.7× on hover, but only while `card.cardSpace is HandStable` **and** the card is face-up (`cardFront.activeSelf` — so a hidden opponent-hand card doesn't lift); no-op in stables / deck / discard. It records the `CardSpace` it captured in and, if the card later changes space while the capture is held (played, discarded, taken into another hand), abandons the capture — undoing only the zoom scale and leaving position/sibling order to the destination's layout. Cosmetic — clicks still route normally.
- **`CardVisuals.Awake`** also adds the data-independent trimmings: a hard `Shadow` on the card front/back and a faint diamond `Image` on the back.
- **`BoardChrome`** — created at runtime by `CardActionExecutor.Awake` (`AddComponent`, like `NeighManager`); needs no scene wiring. In `Start` it discovers the Canvas / `TurnManager` / stables / piles and builds: the board surface (disabling the world-space `GameBoard`), 7 empty-slot wells per unicorn stable, `DECK` / `NURSERY` / `DISCARD` zone labels, a unified top-right HUD panel (turn number + two live win-progress bars, plus the scene's phase text + Skip/Pass button reparented into it), a per-frame active-player glow, and a hidden game-over overlay shown on a `winConditionCount` win. `TurnManager.turnNumber` (display-only counter) feeds the HUD. It scavenges the rounded 9-slice UISprite from an existing scene `Image` (the runtime `Resources.GetBuiltinResource` path is dead in this uGUI version).
- **`HandStable` fan** — `PositionCardsInStable` is parametric arc maths (`fanTotalAngle` / `fanCardSpacing` fields): a lone card is straight up and centred; each extra card opens the fan by `fanTotalAngle / 6°`; centre card upright and highest, outer cards tilt and dip.
- **`EffectChoicePanel`** — the chooser UI for `ChooseEffectAction` (2 buttons) and the executor's `ChoosePlayer` prompt (one button per player; buttons are grown on demand and the plaque resizes). Created at runtime by `CardActionExecutor.Awake` (`AddComponent`, like `BoardChrome`), no scene wiring. Lazily builds a centred cream plaque (title + `UiPalette.Accent` buttons) plus a full-screen `raycastTarget` scrim on the Canvas; shows it while `ChooseEffect` or `ChoosePlayer` is pending, reading `pendingChoiceTitle` + `pendingChoiceLabelA`/`B` or `pendingPlayerChoices`; button i calls `ResolveEffectChoice(i)` or `ResolvePlayerChoice(i)`. Shell only — the executor owns the state.
...
- `TurnSkip`: `Draw`, `Action`, `Turn` (`TurnManager.RequestSkip`)
- `PendingActionType`: `None`, `DiscardCard`, `GiveCard`, `DestroyCard`, `DestroyUnicornCard`, `DestroyUpgradeCard`, `StealCard`, `PlayCardFromHand`, `SacrificeCard`, `TakeFromHand`, `ChooseEffect`, `ReturnToHand`, `MoveUnicorn`, `ChoosePlayer`
...
| MAGIC      | `discardPile` (effect fires before card is moved; Unicorn/Upgrade/Downgrade move first, then fire) |
| NEIGH      | `discardPile`                                       |
...
| Hotseat hand privacy | Done | `HandVisibilityController` — only the active player's hand renders face-up (plus targeted reveals for Neigh response / look-at-hand effects / Peeping Narwhal). See Manager Layer. |
| Skip phase / skip turn | Done | Per-player `TurnManager.RequestSkip`; `SkipPhaseAction`, `ChoosePlayerAction`. Sticky Situation, Unicorn Hangover. |
| Move/loan/return between stables | Done | `ReturnToHandAction`, `MoveUnicornAction` + ChoosePlayer prompt, `ScheduleAtEndOfTurn`, `ISelfSacrificeCondition`. The ChoosePlayer path is unreachable with 2 players — untested until 3+. |
| "Each player" effects | Done | `ForEachPlayerAction` — N-player ready. Used by Cult Leader Unicorn, Safe Sex, Unicorn Acid Trip, Rainbow Shitstorm. |
| "Choose between two effects" | Done | `ChooseEffectAction` + `EffectChoicePanel` — present a player two labelled options, run the picked one (`PrependActions`). Used by Kink Shame, Sex, Drugs, and Unicorns. |
