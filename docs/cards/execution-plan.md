...
Snapshot as of 2026-09-08, after 62/91 cards implemented (Neigh interrupt mechanic + 5 Neigh cards; hand-visibility batch: Hoof Job, Entitled Unicorn, Officer Hornie, Peeping Narwhal + `HandVisibilityController`). Derived by scanning every `docs/cards/card-data/*.md` file not yet marked `impl_status: done` for `NEW:` markers in their **Action Mapping** section.
...
## Blocked (29) — grouped by what's missing, roughly cheapest → most expensive
