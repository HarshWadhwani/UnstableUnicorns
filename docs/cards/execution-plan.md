...
Snapshot as of 2026-09-23, after 68/91 cards implemented (Neigh interrupt mechanic + 5 Neigh cards; hand-visibility batch + `HandVisibilityController`; "choose between two effects" — Kink Shame, Sex, Drugs, and Unicorns + `ChooseEffectAction`/`EffectChoicePanel`; "each player" — Cult Leader Unicorn, Safe Sex, Unicorn Acid Trip, Rainbow Shitstorm + `ForEachPlayerAction`). Derived by scanning every `docs/cards/card-data/*.md` file not yet marked `impl_status: done` for `NEW:` markers in their **Action Mapping** section.
