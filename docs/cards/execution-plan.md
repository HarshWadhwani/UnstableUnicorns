...
Snapshot as of 2026-08-03, after 42/91 cards implemented. Derived by scanning every `docs/cards/card-data/*.md` file not yet marked `impl_status: done` for `NEW:` markers in their **Action Mapping** section.
...
## Blocked (49) — grouped by what's missing, roughly cheapest → most expensive
...
### `SacrificeCardAction.sacrificeAll=false` (PlayerChooses) is an unimplemented stub — 3 cards
`SacrificeCardAction.cs` logs a warning and no-ops when `sacrificeAll=false`. Needs a prompt-driven flow (mirrors `DiscardCardAction`'s `PlayerChooses` branch): active player clicks a card in one of their own stables to sacrifice.
- Double Agent Unicorn (also needs a `targetSubtype=BASIC` filter on the sacrifice, same shape as `StealUnicornAction.targetSubtype`)
- Putting on a Show
- Sadomasocorn
...
2. Implement `SacrificeCardAction.sacrificeAll=false` → Double Agent Unicorn, Putting on a Show, Sadomasocorn.
3. Build `PlayCardFromHandAction`'s subtype extension → Unicorn with Benefits.
4. Build the three small new actions (`BringFromNurseryAction`, `DrawCardAction`, `SearchDeckForTypeAction`) → 7 more cards.
