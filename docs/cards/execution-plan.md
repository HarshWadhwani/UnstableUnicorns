...
## Blocked (39) — grouped by what's missing, roughly cheapest → most expensive

### `DestroyCardAction` has no "destroy all of type" mode — 1 card
- Buck Naked — needs an auto-destroy-all-Upgrades variant (no interactive picking) plus an `Upgrade` value in `DestroyCardAction.TargetStable` (currently only `Any`/`Unicorn`).
...
5. Build `DestroyCardAction`'s destroy-all-of-type mode → Buck Naked.
