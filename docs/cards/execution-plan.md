...
Snapshot as of 2026-08-21, after 45/91 cards implemented. Derived by scanning every `docs/cards/card-data/*.md` file not yet marked `impl_status: done` for `NEW:` markers in their **Action Mapping** section.
...
All 19 cards that were genuinely blocker-free are done (13 no-effect Baby Unicorns rolled into `BabyUnicornCardData` + `BabyUnicornCardDataInstance.asset`; Manscaped Llamacorn, The Bitchiest Unicorn, Semenbiscuit, Homicidal Psychocorn, Fuzzy Hoofcuffs, Free Candy Unicorn as individual cards). Free Candy Unicorn shipped steal-only at first; its leave-stable return clause is now also implemented via the new `IReturnsStolenCardOnLeave` interface (see CLAUDE.md's Card Ability Interfaces table). `SacrificeCardAction.sacrificeAll=false` (PlayerChooses) is now implemented too, which unblocked Double Agent Unicorn, Putting on a Show, and Sadomasocorn — all 3 shipped. Buck Naked (the 4th originally reclassified) still needs `DestroyCardAction`'s destroy-all-of-type mode — see Blocked below.

---

## Blocked (46) — grouped by what's missing, roughly cheapest → most expensive
